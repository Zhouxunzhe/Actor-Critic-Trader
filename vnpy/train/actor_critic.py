...
class ActorCritic(nn.Module):
    def __init__(self, input_dim, hidden_dim, action_dim, batch_size, seq_length):
        super(ActorCritic, self).__init__()
        self.mlp = nn.Linear(5 * input_dim, hidden_dim)
    ...
    def forward(self, x):
        ...
        mlp_out = self.mlp(x)
        action_probs, state_value = self.actor(mlp_out), self.critic(mlp_out)
...
if __name__ == "__main__":
    ...
    model_path = f'../checkpoints/a2c/a2c_model_{stock_name}.pth'
    loss_file_path = f'../checkpoints/a2c/a2c_reward_{stock_name}.csv'
    learning_rate = 1e-4
    num_epochs = 150
    seq_length = 30
    input_dim = 4
    ...
    model = ActorCritic(input_dim, hidden_dim, action_dim, batch_size, seq_length)
